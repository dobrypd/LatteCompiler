...
void InstructionManager::function_prologue()
{
    ...
    Block::instr_ptr_t new_ebp(new instruction::Mov(arg(REGISTER, ESP),
            arg(REGISTER, EBP)));
...
{
    Block::instr_ptr_t restore_esp(new instruction::Mov(
            arg(REGISTER, EBP), arg(REGISTER, ESP)));
...
void InstructionManager::add_to_ESP(int value)
{
    ...
    Block::instr_ptr_t add(new instruction::Add(arg(CONSTANT_FIELD, value * 4),
            arg(REGISTER, ESP)));
    this->add(add);
}
...
void InstructionManager::jump_if(cmp_val_t type, int label_id)
{
    ...
    case NE:
        ...
        break;
    default:
        type_to_str = "ump"; // (j|ump)
        break;
    }

    Block::instr_ptr_t instr(new instruction::ConditionJump(
            Block::ident_prefix + int2str(label_id), type_to_str));
    this->add(instr);
}
...
{
    Block::instr_ptr_t decrease_stack(
            new instruction::Add(arg(CONSTANT_FIELD, 8), arg(REGISTER, ESP)));
    Block::instr_ptr_t cmp(
            new instruction::Cmp(arg(MEMORY, ESP, 4),arg(MEMORY, ESP, 8)));
    this->add(decrease_stack, cmp);
}
...
{
    Block::instr_ptr_t loop(
            new instruction::Loop(Block::ident_prefix + int2str(label_id)));
    this->add(loop);
}
